     ...
     mpCamera(pCamera) ,mpCamera2(nullptr), mbHasPose(false), mbHasVelocity(false)
{
    ...
    for (size_t i(0); i < mvKeys.size(); ++i) {
        cv::Vec3b val = (cv::Vec3b)imS.at<cv::Vec3b>(mvKeys[i].pt.y,mvKeys[i].pt.x);
        if (val != PEOPLE_COLOR)
        {
     ...
     mpCamera(pCamera),mpCamera2(nullptr), mbHasPose(false), mbHasVelocity(false)
{
    ...
    for (size_t i(0); i < mvKeys.size(); ++i) {
      ...
      if (val == PEOPLE_COLOR) {
        mvDynKeys.push_back(mvKeys[i]);
        mDynDescriptors.push_back(mDescriptors.row(i)); // If not worked we can try to inverse condition to consider != case (orig)
      }
      // double prob = ; <- The function for probability vector filling is being called over here
      // mvProb.push_back() <- Over here the vector is being filled
    }
