                ...
                :mbFinishRequested(false),mSkipIndex(SKIP_NUMBER),mSegmentTime(0),imgIndex(0)
{
    ...
    // setenv("PYTHONPATH", strcat(getenv("PYTHONPATH"), ":/home/wfram/testPythonCpp"), 1);
...
void Segment::Run()
{
    ...
    while(1)
    {
        ...
        if(!mImg.empty())
        {
            cout << "Wait for new img time =" << endl;
            if(mSkipIndex==SKIP_NUMBER)
            {
                ...
                // From attempts to interact with Python
                /*                
                CPyInstance hInstance;

	            CPyObject pName = PyUnicode_FromString("segmentation");
	            CPyObject pModule = PyImport_Import(pName);

                const unsigned int nElemImg = mImg.cols * mImg.rows; // Total number of elements
                uchar* arrayImg = new uchar[nElemImg]; // Create an array of apropriate datatype
                std::memcpy(arrayImg, mImg.data, nElemImg * sizeof(uchar)); // Copy the data from the cv::Mat object into the array
                npy_intp arrayImgDim[] = {mImg.rows, mImg.cols}; // The dimensions of the matrix
                PyObject* imgPy = PyArray_SimpleNewFromData(2, arrayImgDim, NPY_UINT8, (void*) arrayImg);

                if(pModule) {
		            CPyObject pFunc = PyObject_GetAttrString(pModule, "segment");
		            if(pFunc && PyCallable_Check(pFunc)) {
			            CPyObject pData = PyObject_CallObject(pFunc, imgPy);
                        uchar *data = (uchar*)PyByteArray_AsString(pData);
                        mImgSegment = cv::Mat(mImg.rows, mImg.cols, CV_8UC3, data); // Are rows cols the same as for mImg after segmentation???
                        // And what about CV_8UC3 ???
                        printf("C: segment(img) = %ld\n", PyLong_AsLong(pData));
		            }
                    else {
			            std::cout << "ERROR: function segment(img)\n" << std::endl;
                    }
                }
                else {
                ...
                */
                ...
                cv::imshow("PSPNet", mImgSegment_color_final);
                cv::waitKey(1);
            }
            mSkipIndex++;
