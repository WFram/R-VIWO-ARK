...
//    return (-392.73 * prob * prob + 348.06 * prob - 77.099);
//  }
//  else {
//    return (-1098.4 * prob * prob + 1477.2 * prob - 528.74);
//  }
//  if (prob <= 0.2) {
//    return (-prob + 1);
//  }
...
//  }
//  else if (prob > 0.45 and prob <= 0.8) {
//    return (-20393 * prob * prob + 18348 * prob - 4127.1);
//  }
//  else {
...
//          const cv::Vec3b& kpSegVal = pKF->mvSegVal[leftIndex];
          ...
          cv::Vec3b kpSegValRight = pKF->mvSegVal[rightIndex];
          obs << kp.pt.x, kp.pt.y;
  ...
  for (size_t i = 0; i < vpMPs.size(); i++) {
    ...
    for (const auto& observation : observations) {
      ...
      if (!pKFi->isBad()) {
        const int leftIndex = get<0>(observation.second);
        cv::KeyPoint kpUn;
        cv::Vec3b kpSegVal, kpSegValRight;
      ...
      if (pMP) {
        ...
        if (!pFrame->mpCamera2) {
          ...
          if (pFrame->mvuRight[i] < 0) {
            ...
            const cv::Vec3b& kpSegVal = pFrame->mvSegVal[i];
            obs << kpUn.pt.x, kpUn.pt.y;
...
//          const cv::Vec3b& kpSegVal = pKFi->mvSegVal[leftIndex];
          ...
          if (rightIndex != -1) {
            ...
            cv::Vec3b kpSegValRight = pKFi->mvSegVal[rightIndex];
            obs << kp.pt.x, kp.pt.y;
...
int Optimizer::OptimizeSim3(KeyFrame* pKF1, KeyFrame* pKF2,
  ...
  for (int i = 0; i < N; i++) {
    ...
    const cv::Vec3b& kpSegVal1 = pKF1->mvSegVal[i];
  ...
  for (auto pMP : lLocalMapPoints) {
    ...
    for (const auto& observation : observations) {
      ...
      if (!pKFi->isBad()) {
        ...
        const cv::Vec3b& kpSegVal = pKFi->mvSegVal[get<0>(observation.second)];
